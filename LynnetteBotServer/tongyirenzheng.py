...
def post_refresh(rtoken,tid,auth,acc,passw):
    ...
    headers = {
        ...
        'Tenant-Id': tid,
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.>'
    }
    data = {
...
#    if True:
        return get_co_and_sa(acc,passw)
    #print(response.json())  # 如果返回的是 JSON 数据，可以使用 json() 方法来解析
    blade = response.json()['access_token']
    uid = response.json()['user_id']
    dept_id = response.json()['role_name'] + ':' + response.json()['dept_id']
    tenant_id = response.json()['tenant_id']
    return [tenant_id,auth,blade,uid,dept_id]
...
def post_api(tid, auth, ticket1):
    ...
    uid = response.json()['user_id']
    dept_id = response.json()['role_name'] + ':' + response.json()['dept_id']
    return [blade,uid,dept_id,rtoken]
...
def get_login_cookies (username, password,exe):
 ...
 #       print(ticket)
        ...
        results = [tenant_id,auth_token,blade[0],blade[1],blade[2],blade[3]]
        #print(results)
        return results
    except requests.exceptions.RequestException as e:
        print("Error:", e)
